...
public class MSTDecoder {
	...
	private ArrayList<String> decodedClasses;
	...
	private void _MSTJumpToNextToken(byte[] data, int[] pos, int[] tokenCount) throws MSTEException {
		...
		while ((!separatorFound)&&(pos[0]<len)){
			if ((char)data[pos[0]]==' ') {pos[0]++;}
			else if ((char)data[pos[0]]==','){pos[0]++; separatorFound=true; tokenCount[0]++;}
			else { throw new MSTEException("_MSTJumpToNextToken: - Bad format (unexpected character before token separator:" + (char)data[pos[0]]);}
		}
		if (!separatorFound){ throw new MSTEException("_MSTJumpToNextToken: - Bad format (no token separator)");}
		while (!nextFound && (pos[0]<len)){
			if ((char)data[pos[0]]==' ') {pos[0]++;}
			nextFound=((char)data[pos[0]]!=' ');
		}
		if (!separatorFound){ throw new MSTEException("_MSTJumpToNextToken: - Bad format (no next token)");}
	...
	private Byte _MSTDecodeChar(byte[] data, int[] pos, String operation) throws MSTEException {
		...
		if (pos[0]==len){	throw new MSTEException("_MSTDecodeChar:" + operation + " (no termination) ");}
		...
		if (sb.length()>0) {
			...
			else{ throw new MSTEException("_MSTDecodeChar: - out of range (" + sb.toString() + ")");} 	
		}
		return value;
	}
	...
	private Float _MSTDecodeFloat(byte[] data, int[] pos, String operation) throws MSTEException {
		...
		if (pos[0]==len){	throw new MSTEException("_MSTDecodeFloat:" + operation + " (no termination) ");}
	...
	private Double _MSTDecodeDouble(byte[] data, int[] pos, String operation) throws MSTEException {
		...
		if (pos[0]==len){	throw new MSTEException("_MSTDecodeDouble:" + operation + " (no termination) ");}
	...
	private Long _MSTDecodeUnsignedInt(byte[] data, int[] pos, String operation) throws MSTEException {
		...
		if (pos[0]==len){	throw new MSTEException("_MSTDecodeInt:" + operation + " (no termination) ");}
		
		if (sb.length()>0) {
			Long l = Long.parseLong(sb.toString());
			if ((l<MSUIntMax)&&(l>=0)) {
				value = Long.parseLong(sb.toString());
			}else{ throw new MSTEException("_MSTDecodeInt: - out of range (" + sb.toString() + ")");}
		}
	...
	private Long _MSTDecodeUnsignedLong(byte[] data, int[] pos, String operation) throws MSTEException {
		...
		if (pos[0]==len){	throw new MSTEException("_MSTDecodeUnsignedLong - " + operation + " (no termination) ");}
	...
	private Integer _MSTDecodeUnsignedShort(byte[] data, int[] pos, String operation) throws MSTEException {
		...
		if (pos[0]==len){	throw new MSTEException("_MSTDecodeUnsignedShort:" + operation + " (no termination) ");}

		if (sb.length()>0) {
			Long l = Long.parseLong(sb.toString());
			if ((l<=MSUShortMax)&&(l>=0)){
				value = Integer.parseInt(sb.toString());
			}else{ throw new MSTEException("_MSTDecodeUnsignedShort: - out of range (" + sb.toString() + ")");}
		}
		return value;
	}
	...
	private Short _MSTDecodeShort(byte[] data, int[] pos, String operation) throws MSTEException {
		...
		if (pos[0]==len){	throw new MSTEException("_MSTDecodeShort:" + operation + " (no termination) ");}

		if (sb.length()>0) {
			Long l = Long.parseLong(sb.toString());
			if ((l<MSShortMax)&&(l>MSShortMin)) {
				value = Short.parseShort(sb.toString());
			}else{ throw new MSTEException("_MSTDecodeShort: - out of range (" + sb.toString() + ")");}
		}
		return value;
	}
	...
	private ArrayList<Object> _MSTDecodeArray(byte[] data, int[] pos, String operation, ArrayList<Object> decodedObjects, ArrayList<String> classes, ArrayList<String> keys, int[] tokenCount, Boolean allowsUnknownUserClasses, HashMap<String,String> nameSpace) throws MSTEException {
		...
		if (count>0){
			for (int i = 0; i<count; i++){
				...
				ret.add(object);
				if (isWeakRef){ throw new MSTEException("_MSTDecodeArray: - Weakly referenced object encountered while decoding an array!" );}							
			}
		}
		return ret;
	}
	...
	private MSCouple _MSTDecodeCouple(byte[] data, int[] pos, String operation, ArrayList<Object> decodedObjects, ArrayList<String> classes, ArrayList<String> keys, int[] tokenCount, Boolean allowsUnknownUserClasses, HashMap<String,String> nameSpace) throws MSTEException{
		...
		if (isWeakRef){ throw new MSTEException("_MSTDecodeCouple: - Weakly referenced object encountered while decoding an MSCouple!" );}	 
		...
		if (classIndex >= 0 && classIndex < classes.size()){
			...
			try{
				...
				if (aClass != null){				
					...
					ret = _MSTDecodeDictionary(data, pos, "_MSTDecodeUserDefinedObject", decodedObjects, classes, keys, tokenCount, true, true ,allowsUnknownUserClasses,nameSpace);
				}
				else{
		...
		while ((pos[0]<len) && (!endStringFound)){
			switch (state){					
				case MSTE_DECODING_STRING_START : {
					if ((char)data[pos[0]] == '"') {pos[0]++; state = MSTE_DECODING_STRING;}
					else { throw new MSTEException("_MSTDecodeString: (wrong starting character) : " + operation);}
					break;
				}
...
*/			         
	...
	private HashMap<String,Object> _MSTDecodeDictionary(byte[] data, int[] pos, String operation, ArrayList<Object> decodedObjects, ArrayList<String> classes, ArrayList<String> keys, int[] tokenCount, Boolean manageReference, Boolean decodingUserClass, Boolean allowsUnknownUserClasses, HashMap<String,String>nameSpace) throws MSTEException {
		...
		if (manageReference){decodedObjects.add(ret);}
			...
			for (int i=0; i<count; i++){
				...
				if (isWeakRef){
					if (decodingUserClass){
						if (manageReference){ throw new MSTEException("_MSTDecodeDictionary - Weakly referenced object encountered while decoding a user class!");}
					}
					else{throw new MSTEException("_MSTDecodeDictionary - Weakly referenced object encountered while decoding a non user class!");}
				}
			}
		}
		return ret;
			...
			while (pos[0]<len){
				switch (state){
					case MSTE_DECODING_ARRAY_START : {
						if ((char)data[pos[0]] == ' ') {pos[0]++;break;}
						if ((char)data[pos[0]] == '[') {pos[0]++; state = MSTE_DECODING_VERSION_START ; break ;} 
						throw new MSTEException("decodeObject:Bad header format (array start) !");
					}
					case MSTE_DECODING_VERSION_START : {
						if ((char)data[pos[0]] == ' ') {pos[0]++;break;}
						if ((char)data[pos[0]] == '"') {pos[0]++; state = MSTE_DECODING_VERSION_HEADER ; break ;} 
					...
					case MSTE_DECODING_VERSION_END : {
						if ((char)data[pos[0]] == '"') {pos[0]++; state = MSTE_DECODING_VERSION_NEXT_TOKEN ; }
						else {throw new MSTEException("decodeObject: Bad header format (version end) !");} 
						break ;
					}
	                ...
	                {	
						...
						switch (state) {
	                        ...
	                        case MSTE_DECODING_CLASSES_NUMBER_NEXT_TOKEN:
	                            if (classesNumber>0) {state = MSTE_DECODING_CLASS_NAME;}
	                        ...
	                        case MSTE_DECODING_CLASS_NEXT_TOKEN:
	                            ...
	                            break ;
	                        case MSTE_DECODING_KEYS_NUMBER_NEXT_TOKEN:
	                            if (keysNumber>0) {state = MSTE_DECODING_KEY_NAME ;}
					...
					case MSTE_DECODING_CRC_START : {
						if ((char)data[pos[0]] == '"') {pos[0]++; state = MSTE_DECODING_CRC_HEADER ; break; }
						else { throw new MSTEException("decodeObject: Bad header format (CRC start)");}
					}
					...
					case MSTE_DECODING_CRC_VALUE : {
						if ((pos[0]+8)>(len-1)) {throw new MSTEException("decodeObject:Bad header format (CRC value) !");}
						StringBuffer sb = new StringBuffer();
					...
					case MSTE_DECODING_CRC_END : {				
						if ((char)data[pos[0]] == '"') {pos[0]++; state = MSTE_DECODING_CRC_NEXT_TOKEN ;}
						else { throw new MSTEException("decodeObject: Bad header format (CRC end)");}
						break;
					}
					case MSTE_DECODING_CLASSES_NUMBER_VALUE : {						
						classesNumber = _MSTDecodeInt(data, pos,"classes number") ;
						if (classesNumber>0) {decodedClasses = new ArrayList<String>(classesNumber);}
					...
					case MSTE_DECODING_KEYS_NUMBER_VALUE : {
						...
						if (keysNumber>0){decodedKeys = new ArrayList<String>(keysNumber);}
					...
					case MSTE_DECODING_ARRAY_END : {
						if ((char)data[pos[0]] == ' ') {pos[0]++;break;}
						if ((char)data[pos[0]] == ']') { state = MSTE_DECODING_GLOBAL_END ; break ;}
