...
public class MSRGBAColor implements MSColor {
    ...
    private static float _MSColorLuminance(int r, int g, int b, int a) {
        return (0.3f * r + 0.59f * g + 0.11f * b) / 255.0f;
    }

    private static boolean _MSIsPaleColor(int r, int g, int b, int a) {
        return _MSColorLuminance(r, g, b, a) > 0.4f ? true : false;
    }

    private static float _Lighter(float x) {
        return (2.0f * (x) * (x) / 3.0f + (x) / 2.0f + 0.25f);
    }

    private static float _Darker(float x) {
        return (-(x) * (x) / 3 + 5.0f * (x) / 6.0f);
    }
